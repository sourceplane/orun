...
| CS6 — cockpit read seam + drill-down + changed view | **In progress** | — | PR1 (#256): `viewmodel` catalog view-models (`CatalogView`/`ComponentRow` + Q2 overlay/filter, `ComponentView`). PR2: `internal/cockpit/catalogread` — the catalog data provider composing objcatalog + the engine's live `FingerprintChangeSource` overlay + the view-models (`CatalogView(withOverlay)`, `ComponentView(key)`). 94.7% coverage. PR3 (live-view ticker, done): the cockpit arms a `refreshInterval` ticker (D-9) in `Init` that silently re-runs `LoadWorkspace` off the UI thread (`workspaceRefreshedMsg`, best-effort — a failed background refresh keeps the current snapshot and never toggles the spinner), so local edits and other processes' writes (an external `orun plan`/`run`, the §0 hook) appear without a keystroke; `ctrl+r` still forces an immediate reload. PR4 (read-side freshness gate, done): `LoadWorkspace` now serves the component list from the object-model catalog at `catalogs/current` when it is **fresh** for the workspace — `services.freshCatalogComponents` recomputes the current source id (`sourcectx.ResolveSourceSnapshot` → `objplan.BuildSourceNode` → `nodes.SourceID`) and compares it to the stored `catalog.SourceID` (design.md §3.1); a clean/unchanged tree reads from state, while a dirty/changed tree (or absent/empty/unreadable store) **falls back to the live intent loader** so uncommitted edits still show immediately. The `CatalogComponentView → ComponentSummary` mapping lives in `services` (consumers.md §3.2). The cockpit does **not** resolve+write-through on a miss (the §0 hook + explicit commands keep the store fresh); that cockpit-side resolve is a tracked follow-up. PR5 (drill-down data layer, done): the component detail now surfaces `spec.change.watches` (`ComponentSummary.Watches`), populated from the object-model catalog mapping (and the legacy loader for parity) and rendered in the inspector — the component-page level read from the state-store read seam. The component→executions join (§5/G-1) is already served by `recentRunsForComponent` over `RunSummary.Components` (scan+filter). **Next (UX review, G-3):** the dedicated drill-down page (catalog→component→job→logs as navigation levels) and the run action (component-scoped, selected env, sealed via `objrun`) — the interactive pieces the spec flags as non-final. |
