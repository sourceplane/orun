...
|---|---|---|
| `ORUN_VERSION` | `latest` | Specific version to install (e.g. `v2.16.0`) |
...
curl -fsSL https://raw.githubusercontent.com/sourceplane/orun/main/install.sh \
  | ORUN_VERSION=v2.16.0 ORUN_INSTALL_DIR=/usr/local/bin sh
```
...
```sh
# Example: Linux amd64
curl -fsSL https://github.com/sourceplane/orun/releases/download/v2.16.0/orun_2.16.0_linux_amd64.tar.gz \
...
```sh
kiox init demo
kiox --workspace demo add ghcr.io/sourceplane/orun:v2.16.0 as orun
...
```sh
docker run ghcr.io/sourceplane/orun:v2.16.0 plan --intent intent.yaml
```

The OCI image is a kiox provider artifact. Use `oras pull ghcr.io/sourceplane/orun:v2.16.0` to pull the raw provider package.
