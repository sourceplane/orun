...
| D-7 | Retire `internal/catalogstore` | **Out of scope (follow-on)** | Convergence is cockpit-scoped here. `catalogstore` still backs `orun catalog list/describe/tree`; full retirement is a separate spec once the object-model catalog read surface is complete. Do **not** regress it. |
...
| **L-3** | Full `internal/catalogstore` retirement (D-7) | Convergence is surface-scoped here; `catalogstore` still backs `orun catalog list/describe/tree` + `ComponentExecutionIndex` | the object-model catalog read surface is complete; forced by L-2 |
