...
| D-7 | Retire `internal/catalogstore` | **Promoted → `specs/orun-legacy-retirement/`** | Convergence is cockpit-scoped here. `catalogstore` still backs `orun catalog list/describe/tree`; full retirement is now planned in its own spec (`specs/orun-legacy-retirement/`, Bucket 1), to land once the object-model catalog read surface is complete. Do **not** regress it. |
...
| **L-3** | Full `internal/catalogstore` retirement (D-7) — **planned in `specs/orun-legacy-retirement/`** | Convergence is surface-scoped here; `catalogstore` still backs `orun catalog list/describe/tree` + `ComponentExecutionIndex` | the object-model catalog read surface is complete; forced by L-2 |
