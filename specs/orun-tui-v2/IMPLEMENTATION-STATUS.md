...
| TR8 | Cloud connect + default flip | ◐ Partial | `internal/tui2/data` (CloudLane), `cmd/orun/command_tui.go` | Default flip landed in #533, then **rolled back to opt-in on 2026-07-17** (product call): v2 ships as `orun tui-next` / `orun tui --next` / ORUN_TUI=next while it soaks; the flip re-lands with TR8.1. Cloud runs lane fixture-tested. Live stage smoke + work SSE/attention/remote-sessions lanes + in-app device flow pending sign-in (needs human browser approval) — TR8.1 |
| TR9 | Cutover (delete v1, rename, hardening) | ◐ Prep landed, deletion gated | `internal/tui` (delete), `internal/tui2`→`internal/tui` | Prep: bare `orun agent` opens the v2 Agents surface when opted in (ORUN_TUI=next). Deletion gated per plan on (a) ≥1 release soaked with the flipped default, (b) TR8.1 cloud lanes, (c) moving `internal/tui/services` out from under `internal/tui` (v2's composer wraps it) |
