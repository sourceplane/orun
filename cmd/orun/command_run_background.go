...
const backgroundChildEnv = "ORUN_BACKGROUND_CHILD"
...
func startBackgroundRun(execID string, store *state.Store, color bool) error {
	...
	if err != nil {
		return fmt.Errorf("locate orun binary: %w", err)
	}
	...
	if err := cmd.Start(); err != nil {
		logFile.Close()
		return fmt.Errorf("start background orun: %w", err)
	}
	pid := cmd.Process.Pid
	// Closing our handle does not affect the child since it has its own fd.
	_ = logFile.Close()

	// Persist the pid so `orun status` can show it.
	...
	fmt.Fprintf(os.Stdout, "  %s orun status --exec-id %s --watch\n",
