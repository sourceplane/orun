...
| NC3 | Cockpit/`status`/`logs` over the folded stream (SSE/long-poll `--follow`); offline local event log + cloud sync | 🟡 Partial — `status` (`LoadRunState`) now **folds the native `…/log` stream** (reads the event log + plan object → `Fold` → ExecState, timestamps recovered from event stamps; falls back to inner on a non-native run). Still missing: SSE/long-poll `--watch`/`--follow` live-tail, offline local event log + cloud sync (pairs **BM3**) |
