...
| Field | Value |
|-------|-------|
| Status | **In progress (coordination cycle now wired, opt-in)** — audited 2026-06-20. NC0 ✅; NC2/NC5 🟡 (wired); NC1/NC4 🟡; NC3 ⛔. `CoordBackend` (`internal/statebackend/coordbackend.go`) implements `Backend` over the native v2 wire — claim/heartbeat/complete + the runnable frontier are conditional appends/reads against the per-run shard, with the lease epoch threaded from `:claim` into `:heartbeat`/`:complete`. `cmd/orun run` selects it when **`ORUN_COORDINATION=v2`** (default off → legacy `remotestate`). `LoadRunState` now folds the native event log (NC3); run create and per-job log read/append still delegate to the v1 client (the native surface doesn't own those yet). Gaps + evidence: `orun-cloud/specs/epics/saas-orun-backend-merge/GAPS.md`. |
| Cluster | **NC** (NC0–NC4) |
...
| NC3 | Cockpit/`status`/`logs` over the folded stream (SSE/long-poll `--follow`); offline local event log + cloud sync | 🟡 Partial — `status` (`LoadRunState`) **folds the native `…/log` stream**; **`status --watch` is now event-driven** (`ReadLog(wait=)` long-poll blocks until an event lands, 15s liveness refresh; OP2 falls back to interval polling). Still missing: `logs --follow` event-driven tail, cockpit live-tail, offline local event log + cloud sync (pairs **BM3**) |
