...
| CS6 — cockpit read seam + drill-down + changed view | **In progress** | — | PR1 (read seam): `internal/cockpit/viewmodel` catalog view-models — `CatalogView`/`ComponentRow` (browse list + changed/affected Q2 overlay + show-only-changed filter) and `ComponentView` (detail page incl. envs + watches), built from `objcatalog` + `affected.Result`. Pure/testable (100% on the new file). **Next:** the freshness gate + refresh hook in `LoadWorkspace`, the live ticker, drill-down wiring, and the run action. |
